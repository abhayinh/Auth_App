...
function Email_verify() {
  ...
  useEffect(() => {
    if (islogin && userdata?.isaccountverify) navigate("/");
  }, [islogin, userdata, navigate]);

  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-blue-200 to-purple-400">
      <img src={assets.logo} onClick={() => navigate("/")} className="absolute left-5 top-5 w-28 cursor-pointer" alt="logo" />
      <form onSubmit={onsubmithandler} className="bg-slate-900 p-8 rounded-lg w-96">
        ...
        <div className="flex justify-between mb-8">
          {Array(6).fill(0).map((_, index) => (
            <input key={index} maxLength="1" ref={e => inputrefs.current[index] = e} onInput={(e) => handleinput(e, index)} onKeyDown={(e) => handlekeydown(e, index)} className="w-12 h-12 bg-[#333A5C] text-white text-xl rounded-md text-center" required />
          ))}
        </div>
