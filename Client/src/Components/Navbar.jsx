...
// import { logout } from "../../../Server/Controller/AuthController.js";

function Navbar() {
  const navigate = useNavigate();
  const { userdata, loading,backendurl,setislogin,setuserdata,islogin } = useContext(AppContext);

  if (loading) return null; // Prevent flash before auth finishes
          ...
          {userdata.name[0].toUpperCase()}
            <div className="absolute top-7 mt-2 w-40 bg-white rounded-xl shadow-lg border border-gray-200 opacity-0 group-hover:opacity-100 invisible group-hover:visible transition-opacity duration-200 z-20">
                <ul className="flex flex-col text-sm text-gray-700">
                    ...
                    {!userdata.isaccountverify && <li onClick={sendverifyotp} className="px-4 py-2 hover:bg-indigo-50 hover:text-indigo-700 cursor-pointer rounded-t-xl">
                    Verify Email</li> }
